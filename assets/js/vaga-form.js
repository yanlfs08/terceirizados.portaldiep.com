...
// ── Estado ────────────────────────────────────────────────────
let dadosOriginais = null; // para diff no audit log
let todosCargos    = [];   // array completo da coleção cargos
...
// ── 1. Carregar empresas e cargos ─────────────────────────────
const loadingEl = document.getElementById("loading-form");
loadingEl.style.display = "block";

try {
  const [empresasSnap, cargosSnap] = await Promise.all([
    getDocs(collection(db, "empresas")),
    getDocs(collection(db, "cargos"))
  ]);

  todosCargos = cargosSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  empresasSnap.docs
    ...
    .forEach(d => {
      ...
      opt.value = d.id;
      opt.textContent = (d.data().nome ?? d.id).toUpperCase();
...
function atualizarCodigoAuto() {
  if (modoEdicao) return; // código fixo no modo edição
  ...
  if (empresaOpt?.value && cargoSel && seq) {
    const empSigla = empresaOpt.textContent.split(/\s+/)[0]; // 1ª palavra do nome
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    // Montar o objeto de dados atualizado
    ...
    const dadosNovos = {
      ...
      numeroSequencial:      parseInt(fSeq.value) || 0,
